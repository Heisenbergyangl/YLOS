...
#define BLOCK_INODES (BLOCK_SIZE / sizeof(inode_desc_t))    // 块 inode 数量(32)
#define BLOCK_DENTRIES (BLOCK_SIZE / sizeof(dentry_t))      // 块 dentry 数量(64)
#define BLOCK_INDEXES (BLOCK_SIZE / sizeof(u16))            // 块索引数量(512)

#define DIRECT_BLOCK (7)                                               // 直接块数量(7)
#define INDIRECT1_BLOCK BLOCK_INDEXES                                  // 一级间接块数量(512)
#define INDIRECT2_BLOCK (INDIRECT1_BLOCK * INDIRECT1_BLOCK)            // 二级间接块数量(262,144)
#define TOTAL_BLOCK (DIRECT_BLOCK + INDIRECT1_BLOCK + INDIRECT2_BLOCK) // 全部块数量 (262,663)
...
inode_t *get_root_inode();          //获取根目录inode
inode_t *iget(dev_t dev, idx_t nr); // 获得设备 dev 的 nr inode
void iput(inode_t *inode);          // 释放 inode
inode_t *new_inode(dev_t dev, idx_t nr); //创建新的 inode

inode_t *named(char *pathname, char **next); // 获取 pathname 对应的父目录 inode
inode_t *namei(char *pathname);              // 获取 pathname 对应的 inode

inode_t *inode_open(char *pathname, int flag, int mode);    //打开文件，返回 inode

int inode_read(inode_t *inode, char *data, u32 len, off_t offset); // 从 inode 的 offset 处，读 len 个字节到 buf
int inode_write(inode_t *inode, char *data, u32 len, off_t offset); // 从 inode 的 offset 处，将 data 的 len 个字节写入磁盘
